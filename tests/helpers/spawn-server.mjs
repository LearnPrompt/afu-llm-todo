...
async function spawnPlannerServer({ calendarProvider = 'none' } = {}) {
  ...
  const larkStub = path.join(workRoot, 'lark-cli');
  await fs.writeFile(larkStub, '#!/bin/sh\necho "lark-cli stub: simulated failure" >&2\nexit 1\n');
  await fs.chmod(larkStub, 0o755);
